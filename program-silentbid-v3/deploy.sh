...
# Deploy script for silentbid_v3.aleo to testnet
PRIVATE_KEY="$1"
if [ -z "$PRIVATE_KEY" ]; then
    if [ -f "../.env" ]; then
        set -a; source "../.env"; set +a
        PRIVATE_KEY="$ALEOPVTKEY"
    fi
fi
if [ -z "$PRIVATE_KEY" ]; then
    echo "Error: Private key required. Set ALEOPVTKEY in .env or pass as argument"
    exit 1
fi
...
echo "Building silentbid_v3.aleo..."
leo build
if [ $? -ne 0 ]; then echo "Build failed."; exit 1; fi

echo "Deploying silentbid_v3.aleo to testnet..."
